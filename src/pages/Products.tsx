...
interface FarmerPublicProfile { id: string; farm_name: string | null; state: string | null; verification_status: string | null; user_id: string | null; }
...
const Products = () => {
  ...
  useEffect(() => {
    ...
    const queryStateParam = searchParams.get('state') as State;
    if (queryStateParam && STATES.some(s => s.value === queryStateParam)) {
      setSelectedState(queryStateParam);
    }
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      if (farmersError) {
        console.warn('Farmer public profile lookup failed; continuing with product-only fallback', farmersError);
      }

      const farmerMap = new Map((farmersData || []).map(f => [f.id, f]));

      const nextProducts = (productsData || []).map((product) => {
        const fallbackFarmer: FarmerPublicProfile = {
          id: product.farmer_id,
          farm_name: 'Farm listing',
          state: product.state,
        ...
        return {
          ...
          farmer: farmerMap.get(product.farmer_id) || fallbackFarmer,
        };
      });
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(p => {
      ...
      const verificationStatus = p.farmer?.verification_status || '';
      const matchesVerified = !verifiedOnly || verificationStatus === 'verified' || verificationStatus === 'approved';
  ...
  const transformedProducts = useMemo(() => filteredProducts.map(p => ({
    ...
    farmerId: p.farmer?.id || p.farmer_id, farmerName: p.farmer?.farm_name || 'Farm listing', farmName: p.farmer?.farm_name || 'Farm listing',
    state: (p.state || p.farmer?.state || 'all') as State, available: p.available_quantity,
    isVerified: (p.farmer?.verification_status || '') === 'verified' || (p.farmer?.verification_status || '') === 'approved',
  })), [filteredProducts]);

...
import { Clock, CheckCircle, Users, Flag, MapPin, Bell } from 'lucide-react';

export default function AdminDashboard() {
  const { user, loading: authLoading } = useAuth();
  const navigate = useNavigate();
  const [stats, setStats] = useState({ pendingVerifications: 0, totalFarmers: 0, pendingReports: 0, unreadNotifications: 0 });
  ...
  useEffect(() => {
    const checkAdminAndFetchStats = async () => {
      ...
      const [pendingRes, farmersRes, reportsRes, notifRes] = await Promise.all([
        ...
        supabase.from('admin_notifications' as any).select('id', { count: 'exact', head: true }).eq('is_read', false),
      ]);
  ...
  const statCards = [
    ...
    { title: 'Notifications', value: stats.unreadNotifications > 0 ? stats.unreadNotifications : 'View', icon: Bell, color: 'text-blue-600', bgColor: 'bg-blue-100', link: '/admin/notifications' },
      ...
      <div className="container py-8">
              ...
              <Card className="hover:shadow-card-hover transition-shadow cursor-pointer">
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-foreground flex items-center gap-2">
                    {stat.value}
                    {stat.title === 'Notifications' && stats.unreadNotifications > 0 && (
                      <Badge variant="destructive" className="text-xs">{stats.unreadNotifications} new</Badge>
                    )}
                  </div>
                </CardContent>
              </Card>
        ...
        <Card>
          ...
          <CardContent className="flex flex-wrap gap-3">
            ...
            <Button variant="outline" asChild><Link to="/admin/notifications">Notifications</Link></Button>
            ...
            <Button variant="outline" asChild><Link to="/admin/locations"><MapPin className="h-4 w-4 mr-2" />Manage Locations</Link></Button>
          </CardContent>
        </Card>
      </div>
    </Layout>

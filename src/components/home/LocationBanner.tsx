...
export const LocationBanner = () => {
  ...
  const handleStateChange = (newState: State) => {
    if (newState !== selectedState) {
      // If cart has items and changing state, warn user
      if (items.length > 0 && newState !== 'all' && selectedState !== 'all') {
        // For simplicity, just change state - products from different states will still be in cart
        // The checkout process will handle state-specific delivery
      }
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm text-muted-foreground">Showing products in</p>
          ...
          <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
            <DropdownMenuTrigger asChild>
              <Button 
                variant="outline" 
                className="gap-2 border-primary/30 hover:bg-primary/10 hover:border-primary flex-shrink-0"
              >
